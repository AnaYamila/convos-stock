...
async function borrarVentaEnSheets(fila) {
  ...
  const t = setTimeout(() => ctrl.abort(), 30000);   // 30s de límite
  let resp;
  try {
    resp = await fetch(config.appsScriptUrl, {
      method:  'POST',
      headers: { 'Content-Type': 'text/plain' },
      body:    JSON.stringify(payload),
      signal:  ctrl.signal,
    ...
    throw new Error('La operación tardó demasiado. Probá de nuevo o tocá 🔄.');
  } finally {
    clearTimeout(t);
  }
  if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
